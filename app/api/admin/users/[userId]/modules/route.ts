...
export async function PATCH(
  ...
  try {
    ...
    if (adminUser.tipo !== "ADMIN") {
      throw new ForbiddenError("Acesso negado. Apenas administradores podem atualizar permissões.");
    }
    ...
    const { modulo_importacao, modulo_livre, modulo_sala } = body;
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, error: "Nenhum módulo foi especificado para atualização." },
        { status: 400 }
      );
    }
    ...
    if (modulo_importacao !== undefined) updateData.modulo_importacao = modulo_importacao;
